...
class IORedisStorage
  constructor: (@instance, @initSettings, options) ->
    @originalId = @instance.id
    parser.load options, options, @
    @isReady = false

    @connection = @_groupConnection ? new IORedisConnection @clientOptions, @Promise, @instance.Events
  ...
  _loadAllScripts: () ->
    ...
    else Scripts.names.forEach (name) =>
      @connection.client.defineCommand name, { lua: Scripts.payload(name) }
      @Promise.resolve()
  ...
  runScript: (name, args) ->
    if !@isReady then @Promise.reject new BottleneckError "This limiter is not done connecting to Redis yet. Wait for the '.ready()' promise to resolve before submitting requests."
      ...
      new @Promise (resolve, reject) =>
        arr = [keys.length].concat keys, args, (err, replies) ->
          if err? then return reject err
          return resolve replies
        @instance.Events.trigger "debug", ["Calling Redis script: #{name}.lua", args]
        @clients.client[name].bind(@clients.client).apply {}, arr
      .catch (e) =>
